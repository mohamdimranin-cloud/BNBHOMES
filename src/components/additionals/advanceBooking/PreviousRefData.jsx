...
const PreviousRefData = ({ bookingData }) => {
  ...
  useEffect(() => {
    const addBookingData = async () => {
      try {
        ...
        setAdvanceBooking({
          prev_booking_ref: bookingData.advanceBooking.advance_booking_ref || "",
          guest_name: bookingData.advanceBooking.guest_name || "",
          mobile_num: bookingData.advanceBooking.mobile_num || "",
          whatsapp_num: bookingData.advanceBooking.whatsapp_num || "",
          email_id: bookingData.advanceBooking.email_id || "",
          num_of_guest: Number(bookingData.advanceBooking.num_of_guest) || 0,
          guest_type: bookingData.advanceBooking.guest_type || "",
          checkin_date: dayjs(bookingData.advanceBooking.checkin_date) || dayjs(today),
          checkout_date: dayjs(bookingData.advanceBooking.checkout_date) || dayjs(tomorrow),
          num_of_nights: Number(bookingData.advanceBooking.num_of_nights) || 0,
          room_with_gst: Number(bookingData.advanceBooking.room_with_gst) || 0,
          total_amount: Number(bookingData.advanceBooking.total_amount) || 0,
          discount_any: bookingData.advanceBooking.discount_any || "",
          final_amount: Number(bookingData.advanceBooking.final_amount) || 0,
          advance_amount: bookingData.advanceBooking.advance_amount || "",
          paid_via: bookingData.advanceBooking.paid_via || "",
          paid_reference: bookingData.advanceBooking.paid_reference || "",
          balance_amount: Number(bookingData.advanceBooking.balance_amount) || 0,
          remarks: bookingData.advanceBooking.remarks || "",
        });
        ...
        setRooms(
          bookingData.rooms.map((room) => ({
            ...
            num_of_rooms: Number(room.num_of_rooms) || 0,
          }))
        );
    ...
    const getAdvanceBookingRef = async () => {
      try {
        ...
        if (res.status === 200) {
          ...
          // Backend returns { nextRef: "AD001/25" } — use directly, no increment needed
...
export default PreviousRefData